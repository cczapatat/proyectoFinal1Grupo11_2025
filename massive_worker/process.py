...
topic_path = publisher_manufactures.topic_path(project_id, manufactures_name_pub)

publisher_retry_attemp = pubsub_v1.PublisherClient()
topic_path = publisher_retry_attemp.topic_path(project_id, attemps_name_pub)
...
def __create_manufacture_proccessed__(process_id, file_id, user_email, future, current_batch, number_of_batches) -> bool:
    try:
        ...
        print('[Process Attemps][__create_manufacture_proccessed__] process_id: {}, manufacture_batch: {}'.format(str(process_id), str(manufacture_batch)))

        return manufacture_batch
    except Exception as ex:
        session.rollback()
        print('[Process Attemps][__create_manufacture_proccessed__] process_id: {}, error {}'.format(str(process_id), str(ex)))
        return False
...
def __create_and_process__(operation, entity, process_id, file_id, user_email) -> Union[Attempt, bool]:
    ...
    if not attempt:
        print('[Process Attemps][__create_and_process__] process_id: {}, error {}'.format(str(process_id), 'Error creating attempt'))
        return Attempt()    
...
def publish_massive_manufactures(process_id, file_id, user_email, json_data):
    ...
    for i in range(0, total_rows, batch_size):
        ...
        print(f"[Process Manufactures] Publishing to {topic_path} the message: {data_str}")

        future = publisher_manufactures.publish(topic_path, data_str)
...
def process_attemps(message):
    ...
    try:
        ...
        if json_data_file is None:
            ...
            future = publisher_retry_attemp.publish(topic_path, message.data)
            print(future.result())
        ...
        print(f"[Process Attemps] attempt {str(attempt.id)} with process_id {str(attempt.process_id)} by user {attempt.user_email} created")
